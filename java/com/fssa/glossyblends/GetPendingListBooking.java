...
/**
 * Servlet implementation class GetPendingListBooking
 */
@WebServlet("/GetPendingListBooking")
public class GetPendingListBooking extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GetPendingListBooking() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	 ...
	 HttpSession session=request.getSession();
		...
		if(email!=null && loggedIn) {
			...
			try {
				List<Booking> listBooking=book.getPendingAppoinmentsById(id);
				request.setAttribute("listOfPending",listBooking);
				request.getServletContext().getRequestDispatcher("/UserAppoinment.jsp").forward(request, response);				
